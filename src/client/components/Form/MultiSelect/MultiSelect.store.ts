...
import { observable, observe, set, computed, action } from "mobx";
import { FormTheme, FormItemModel, FormIntegrationProps } from "../Form.types";
import { validate, ValidationType } from "client/utils/validation";
...
import OptionStore from "client/components/Form/Option/Option.store";
import ListStore from "client/components/List/List.store";

import KeybindingStore from "client/core/common/Keybinding.store";
...
export default class MultiSelectStore<T = CommonMap> implements FormIntegrationProps, FormItemModel {
    ...
    constructor(props: InitProps<T>) {
        const { options = [], defaultValue, getOptionLabel, getOptionValue, ...restProps } = props;
        set(this, restProps);
