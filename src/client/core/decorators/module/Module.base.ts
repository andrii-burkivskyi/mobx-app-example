...
import { computed, observe, action, observable } from "mobx";
import { AsyncModuleExtractor, ClassExtractor, AsyncLazyConstructor, AsyncModuleItem, SyncModuleExtractor, SyncLazyConstructor } from "./module.extractor";
...
class ModuleBase {
    ...
    finishModuleLoading = () => {};
    isModuleLoading = new Promise((resolve) => {
        this.finishModuleLoading = resolve;
    });

    isLoading = async () => {
        console.log("loading start");
        console.log(this.isModuleLoading);
        await this.isModuleLoading;
    }

    parent?: ModuleBase;
    @observable View: Nullable<Constructable<React.Component<any>>> = null;
    @observable model: Nullable<Object> = null;
    ...
    @action private asyncInitModules = async () => {
        ...
        modulesLists.forEach(async (list) => {
    ...
    private injectModelDependencies = (ModelConstructor: Constructable<Object>) => {
    ...
    private injectDependencies = (ClassConstructor: Constructable<Object>, constructors: Constructable<Object>[] = []): Object => {
