...
export class LocationService {
    ...
    isValidRoute = createTransformer((props: { route: string, options?: TokensToRegexpOptions}) => {
        return Boolean(matchUrl(this.pathname, props.route, props.options))
    });

    routePrams = createTransformer<string, CommonMap>((route: string) => {
        return matchUrl(this.pathname, route, { end: false }) ?? DEFAULT_OBJECT;
    })
