...
import { ICommonFiltersMapInput, ICommonFilterInput } from "../models/app/filter";
import { PaginationInput, IPaginationInput } from "../models/app/pagination/pagination.input";
...
const isCommonFiltersMap = (where?: any): where is ICommonFiltersMapInput =>
...
export const getSelectQueryBuilder = <E, P extends any>(repository: Repository<E>, input: PaginationInput<P>, joins: IJoinProps[] = []) => {
    ...
    if (isCommonFiltersMap(input.filter)) {
        Object.entries<ICommonFilterInput>(input.filter).forEach(([key, value]) => {
            buildQueryFilter(input.filter, query, (key: string) => ({
                linkName: `q.${key}`,
                varName: key
            }))
        })
