...
exports.getSelectQueryBuilder = (repository, input, joins = []) => {
    ...
    if (isCommonFiltersMap(input.filter)) {
        Object.entries(input.filter).forEach(([key, value]) => {
            buildQueryFilter(input.filter, query, (key) => ({
                linkName: `q.${key}`,
                varName: key
            }));
        });
...
exports.getPaginationResponse = (repository, input, joins = []) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    ...
    const [entity, count] = yield query.getManyAndCount();
    return {
        items: entity,
        totalItems: count,
        take: input.take,
